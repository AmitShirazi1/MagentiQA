...
/**
 * Fixed introductory paragraph for the verification report. It is NOT part of the
 * editable per-version setup — it always opens the report's setup page, for every
 * version of every project, and is rendered directly by lib/pdf.js.
 */
const SETUP_PREAMBLE = `Each test case is self-contained and executed independently. Any special setup required by a particular test is specified within that test case. Certain tests may additionally require specific hardware, system settings, configurations, or pre-conditions.\n`;

/**
 * The editable standard verification setup, shown at the top of every version and
 * printed (after the fixed preamble above) on the opening page of the signed
 * verification report. It is the baseline a version starts from: the first version
 * of a project falls back to this text, and each later version inherits its
 * predecessor's (see routes/projects.js). A QA Engineer can override it per
 * version from the version view.
 */
const DEFAULT_SETUP_TEXT = `Configuration
...
module.exports = { saveSetups, uniqueSetupId, SETUP_PREAMBLE, DEFAULT_SETUP_TEXT };
