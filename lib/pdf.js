...
/* Setups can have many columns / long values — shrink only this table (fixed
   layout + wrapping) so it fits the page width without scaling the whole report. */
.setups-table { font-size: 8px; table-layout: fixed; }
.setups-table th, .setups-table td { padding: 4px 5px; word-break: break-word; overflow-wrap: anywhere; }
.setups-table .badge { font-size: 7.5px; padding: 1px 4px; }
.setups-table .exec-col { width: 86px; }
...
function renderSetupsSection(test, setups, executions) {
  ...
  // Its own class keeps this (potentially wide, many-column) table at a smaller
  // font without shrinking the rest of the report.
  return `
    ...
    <table class="steps-table setups-table">
      <thead><tr>${cols.map(c => `<th>${esc(c)}</th>`).join('')}<th class="exec-col">Executed</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}
