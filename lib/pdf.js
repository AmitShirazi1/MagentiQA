...
async function generateVersionReport(versionId) {
  ...
  const filename  = `MagentiQA_Report_${versionId}_${Date.now()}.pdf`;
...
function buildReportHtml(versionId) {
  ...
  const enriched = vTests.map(vt => {
    ...
    return { vt, test, tags, steps, lastExec, stepResults, sigs, executor };
  });
  ...
  const overviewRows = enriched.map(({ vt, test, executor, lastExec }) => `
    <tr class="test-row ${vt.status.toLowerCase()}">
      ...
      <td><span class="badge ${vt.status.toLowerCase()}">${vt.status.replace('_', ' ')}</span></td>
  ...
  const detailPages = enriched.map(({ vt, test, tags, steps, lastExec, stepResults, sigs, executor }) => {
    const srMap = {};
    ...
    const stepsHtml = steps.map((s, i) => {
      const sr    = srMap[s.id] || {};
      const res   = sr.result || '—';
      const resClass = res === 'PASS' ? 'pass' : res === 'FAIL' ? 'fail' : '';
      return `
        <tr>
          <td class="step-num">${s.order || i + 1}</td>
          <td>${esc(s.action)}</td>
          <td class="text-secondary">${esc(s.expectedResult || '—')}</td>
          <td><span class="badge ${resClass}">${esc(res)}</span></td>
      ...
      <div class="detail-page page-break">
        <div class="detail-header">
          ...
          <div class="result-badge">
            <span class="badge ${vt.status.toLowerCase()}">${vt.status.replace('_', ' ')}</span>
          </div>
        </div>
...
/* ── Cover page ── */
.cover { padding: 50px 50px 30px; border-bottom: 4px solid #1e3a5f; }
.cover-logo { font-size: 20px; font-weight: 800; color: #1e3a5f; letter-spacing: 2px; text-transform: uppercase; }
.cover-subtitle { font-size: 10px; color: #666; margin-top: 3px; letter-spacing: 1px; }
.cover-title { font-size: 26px; font-weight: 700; color: #1e3a5f; margin-top: 28px; }
.cover-meta { margin-top: 18px; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; }
.cover-meta .label { color: #666; font-weight: 600; text-transform: uppercase; font-size: 9px; letter-spacing: 1px; }
.cover-meta .value { color: #1a1a2e; margin-top: 2px; font-size: 12px; }

/* ── Sections ── */
section { padding: 24px 50px; }
h2 { font-size: 13px; font-weight: 700; color: #1e3a5f; border-bottom: 1px solid #e2e8f0;
...
.stat .lbl { font-size: 9px; text-transform: uppercase; letter-spacing: 1px; color: #64748b; margin-top: 2px; }
...
th { background: #1e3a5f; color: #fff; font-weight: 600; text-transform: uppercase;
     font-size: 9px; letter-spacing: 0.5px; padding: 7px 9px; text-align: left; }
td { padding: 6px 9px; border-bottom: 1px solid #f1f5f9; vertical-align: top; }
tr:nth-child(even) td { background: #f8fafc; }
.tags-cell { font-size: 9px; color: #64748b; }
...
.badge.blocked                { background: #fef9c3; color: #854d0e; }
...
.sig-box .role { font-size: 9px; text-transform: uppercase; letter-spacing: 1px; color: #64748b; font-weight: 600; }
.sig-box .name { font-size: 13px; font-weight: 700; margin-top: 4px; }
.sig-box .date { font-size: 10px; color: #64748b; margin-top: 2px; }
.sig-box .hash { font-size: 8px; color: #94a3b8; font-family: monospace; margin-top: 5px; word-break: break-all; }
...
.detail-header h3 { font-size: 15px; font-weight: 700; color: #1e3a5f; margin-top: 3px; }
...
.tag { display: inline-block; background: #eff6ff; color: #1e40af; border: 1px solid #bfdbfe;
       border-radius: 3px; padding: 1px 6px; font-size: 9px; margin-right: 4px; }
...
.meta-label { font-size: 9px; text-transform: uppercase; letter-spacing: 0.8px; color: #64748b; font-weight: 600; margin-bottom: 3px; }
.meta-value { font-size: 10px; color: #1a1a2e; white-space: pre-wrap; }
...
.exec-footer { margin-top: 10px; font-size: 9px; color: #64748b; display: flex; gap: 20px; }
...
<div class="cover">
  <div class="cover-logo">MagentiQA</div>
  <div class="cover-subtitle">Verification Management Platform</div>
  ...
  <div class="cover-meta">
    ...
    <div><div class="label">Regulatory</div><div class="value">ISO 13485 · FDA 21 CFR Part 820</div></div>
  </div>
</div>
...
<section>
  ...
  <div class="stats-grid">
    ...
    <div class="stat pass"><div class="num">${stats.passed}</div><div class="lbl">Passed</div></div>
    <div class="stat partial"><div class="num">${stats.partial}</div><div class="lbl">Partial</div></div>
    <div class="stat fail"><div class="num">${stats.failed}</div><div class="lbl">Failed</div></div>
    <div class="stat pending"><div class="num">${stats.inProgress}</div><div class="lbl">In Progress</div></div>
    <div class="stat"><div class="num">${stats.notStarted}</div><div class="lbl">Not Started</div></div>
  </div>
</section>
...
<div class="page-footer">
  <span>CONFIDENTIAL — For Internal Use Only · ISO 13485 · FDA 21 CFR Part 820</span>
...
module.exports = { generateVersionReport };
