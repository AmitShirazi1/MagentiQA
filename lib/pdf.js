...
async function generateVersionReport(versionId) {
  ...
  const filename  = `MagentiqEye_Report_${versionId}_${Date.now()}.pdf`;
...
function buildReportHtml(versionId) {
  ...
  // Always show Total / Passed / Partially Passed / Failed; show In Progress and
  // Not Started only when they're non-zero (a clean report hides empty buckets).
  const statCards = [
    { num: stats.total,   label: 'Total',            cls: '' },
...
/** Internal verdict → human label shown in the Status column. */
function statusLabel(s) {
  return { PASSED: 'Passed', FAILED: 'Failed', BLOCKED: 'Blocked', PENDING: 'Pending' }[s] || '';
}
...
function renderSetupsSection(test, setups, executions) {
  ...
  const rows = setups.map(s => {
    ...
    const status = ex ? ex.result : (s.status || '');
    const tester = ex ? (db.users.findById(ex.executorId)?.name || '—') : (s.testerName || '');
    const cells = cols.map(c => {
      if (meta.statusColumn && c === meta.statusColumn) {
        return `<td>${status ? `<span class="badge ${String(status).toLowerCase()}">${esc(statusLabel(status) || status)}</span>` : '—'}</td>`;
      }
...
module.exports = { generateVersionReport, buildReportHtml };
