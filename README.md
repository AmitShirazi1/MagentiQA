...
| **VersionTest** | Links a test definition to a version. Its stored `status` is the last **signed** outcome (`NOT_STARTED`, `PASSED`, `FAILED`, or — for setup-tracked — `IN_PROGRESS`/`PARTIAL` rolled up across setups). The **version view shows an *effective* status**: an active shared draft (see ExecutionDraft) raises it to `IN_PROGRESS`, or `BLOCKED` if any recorded step is marked blocked. Also carries the workflow state (`DRAFT`, `IN_REVIEW`, `APPROVED`). |
